...
public class Huurder extends Gebruiker implements HuurderInterface{

    public ArrayList <VerhuurFragment> fragment;
    private Licentie licentie;

    public Huurder(UUID id, String userName, String emailAddress, String password, String firstName, String lastName, String phoneNumber) {
    ...
    @Override
    public void getHuurHistorie() {
        for (VerhuurFragment fragment: getFragment())
        {
            System.out.println("ID: " + fragment.getId());
            System.out.println("Starttijd: " + fragment.getStartTijd());
            System.out.println("Eindtijd: " + fragment.getEindTijd());
            System.out.println("");
        }
    }
    ...
    public void autoHuren() throws ParseException {
        ...
        if (inputDateIsValid) {
            ...
            if (startDatum.before(VerhuurFragment.getTodaysDate()) || eindDatum.before(VerhuurFragment.getTodaysDate())) {
                System.out.println("Gegeven datum is ongeldig");
            } else {

                int count = -1;
                ArrayList < ModernAuto > beschikbaarLijst = new ArrayList < ModernAuto > ();
                System.out.println("Dit zijn de beschikbare autos op de gegeven start-eind datum:");

                for (ModernAuto auto: ModernAuto.getAutos()) {
                    if (auto.getHired() == false) {
                        count++;
                ...
                ModernAuto selectAuto = beschikbaarLijst.get(inputAutoKeuze);

                if (this.hasLicentie() && this.getCurrentActiveRentalAmount() < this.getLicentie().getMaxAuto()) {
